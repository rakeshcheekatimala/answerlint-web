...
import { MAX_VISIBILITY_RUNS_PER_BENCHMARK, VISIBILITY_PROJECT_TOKEN_HEADER } from "@/lib/visibility/constants";
import {
...
export function VisibilityWorkspaceClient({ initialProjectId }: Props) {
  ...
  const plannedDraftRuns = Math.max(1, 3 * surfaces.length * repeats);
      ...
      <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-score-high">
            <PulseIcon /> Evidence-first AI visibility
          </div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">Build a compact, approval-first benchmark. AnswerLint records the prompt, run policy, answer, citations, source checks, and the exact page your team can improve.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
          ...
          <HeroStat label="Run budget" value="≤ 36" body="A firm cap keeps experiments intentional." />
        </div>
      </div>
    </section>
    ...
    <section className={`${cardClass} mt-6 overflow-hidden`}>
      ...
      <form className="p-5 sm:p-8" onSubmit={form.handleSubmit(onSubmit)}>
        <div className="grid gap-x-5 gap-y-6 lg:grid-cols-2">
          ...
          <div className="lg:col-span-2"><Field label="Measurement surface" hint="Results are labelled by how they were collected. Coming-soon surfaces are never shown as zero visibility."><div className="mt-2 grid gap-3 md:grid-cols-2">{VISIBILITY_SURFACE_DEFINITIONS.map((surface) => { const selected = surfaces.includes(surface.surface); const ready = surface.availability === "available"; return <button key={surface.surface} type="button" disabled={!ready} onClick={() => onToggleSurface(surface.surface)} className={`group relative min-h-28 rounded-xl border p-4 text-left transition ${selected ? "border-ink bg-ink text-white" : ready ? "border-border bg-paper hover:border-ink" : "cursor-not-allowed border-border bg-paper-muted text-ink-muted"}`} aria-pressed={selected}><div className="flex items-start justify-between gap-3"><span className="font-semibold">{surface.label}</span><span className={`rounded-full border px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em] ${selected ? "border-white/20 text-score-high" : "border-border-strong text-ink-muted"}`}>{ready ? "Controlled beta" : "Coming soon"}</span></div><p className={`mt-2 text-xs leading-5 ${selected ? "text-white/65" : "text-ink-muted"}`}>{surface.description}</p></button>; })}</div></Field></div>
          ...
          <Field label="Run policy" hint="Repeated fresh API runs show stability. Scheduling is intentionally configured after the first benchmark."><div className="mt-2 grid grid-cols-2 gap-3"><label className="rounded-xl border border-border bg-paper p-3 text-xs font-semibold text-ink">Samples<select className="mt-2 w-full bg-transparent text-sm font-semibold outline-none" {...form.register("runtimePolicy.repeatRuns", { valueAsNumber: true })}><option value={1}>1 exploratory</option><option value={3}>3 evidence-grade</option><option value={5}>5 high variance</option></select></label><label className="rounded-xl border border-border bg-paper p-3 text-xs font-semibold text-ink">Search<select className="mt-2 w-full bg-transparent text-sm font-semibold outline-none" {...form.register("runtimePolicy.searchMode")}><option value="search_enabled">Search required</option><option value="model_only">Model only</option></select></label></div></Field>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-border pt-5"><p className="max-w-xl text-sm leading-6 text-ink-muted">You review the entity and prompt cohort before AnswerLint makes a durable provider call. The initial baseline is capped at {MAX_VISIBILITY_RUNS_PER_BENCHMARK} runs.</p><button type="submit" disabled={isSubmitting} className="inline-flex items-center gap-2 rounded-xl bg-ink px-5 py-3 text-sm font-semibold text-white transition hover:bg-ink-muted disabled:cursor-not-allowed disabled:opacity-60">{isSubmitting ? "Creating workspace…" : "Create evidence workspace"}<ArrowIcon /></button></div>
      </form>
    </section>
  </>;
}
...
function SettingsPanel({ project, report }: { project: VisibilityProject; report: VisibilityWorkspaceReport }) {
  return <div className="grid gap-4 lg:grid-cols-2"><article className="rounded-2xl border border-border bg-paper p-5"><p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-muted">Controlled-run method</p><dl className="mt-5 space-y-4"><Definition label="Measurement lane" value="Controlled provider API run" /><Definition label="Search policy" value={project.intake.runtimePolicy.searchMode === "search_enabled" ? "Search required" : "Model only"} /><Definition label="Repeat policy" value={`${project.intake.runtimePolicy.repeatRuns} fresh run${project.intake.runtimePolicy.repeatRuns === 1 ? "" : "s"} per prompt`} /><Definition label="Benchmark cap" value={`${MAX_VISIBILITY_RUNS_PER_BENCHMARK} provider calls per run`} /></dl></article><article className="rounded-2xl border border-border bg-paper-muted p-5"><p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-muted">Evidence safeguards</p><ul className="mt-5 space-y-3 text-sm leading-6 text-ink-muted"><li className="flex gap-3"><CheckIcon />No unconfigured surface is reported as zero visibility.</li><li className="flex gap-3"><CheckIcon />Source claims require independently resolved public URLs.</li><li className="flex gap-3"><CheckIcon />The raw answer and provider-source manifest are retained as private artifacts.</li><li className="flex gap-3"><CheckIcon />{report.measurementCoverage.completedRuns ? "Reported metrics exclude incomplete groups." : "Metrics remain blank until evidence passes the gate."}</li></ul></article></div>;
}
