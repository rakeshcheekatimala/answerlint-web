...
function tokenMatchesHash(token: string | undefined, storedHash: string | null | undefined) {
...
export async function storeVisibilityProjectBestEffort(
  ...
  if (!isSupabaseAdminConfigured()) return localProject(project, "skipped", editToken);
  ...
  try {
    ...
    if (error || !data) throw new Error(error?.message ?? "Unable to store project.");
    ...
    return localProject(project, "failed", editToken, formatVisibilityStorageError(error));
  }
}
...
export async function getVisibilityProject(
  ...
  if (!tokenMatchesHash(editToken, project.owner_token_hash)) {
    throw new VisibilityProjectAccessError("You do not have permission to open this project.");
  }
...
async function loadVisibilityProjectRelations(
  ...
  const [cardResult, assetsResult, topicsResult, promptsResult, competitorsResult] =
    await Promise.all([
      supabase
      ...
      supabase
        ...
        .eq("project_id", project.id),
      supabase.from("visibility_topics").select(topicSelect).eq("project_id", project.id),
      supabase.from("visibility_prompts").select(promptSelect).eq("project_id", project.id),
      supabase.from("visibility_competitors").select("name,url").eq("project_id", project.id),
    ]);

  if (cardResult.error || !cardResult.data) {
    throw new Error(cardResult.error?.message ?? "AI Visibility card not found.");
  }
  if (assetsResult.error || topicsResult.error || promptsResult.error || competitorsResult.error) {
  ...
  return fromRows({
    ...
    competitors: (competitorsResult.data ?? []) as Array<{ name: string; url: string | null }>,
  });
}
...
export async function updateVisibilityProjectApprovals(
  ...
  if (error || !data || !tokenMatchesHash(editToken, data.owner_token_hash)) {
    throw new VisibilityProjectAccessError("You do not have permission to modify this project.");
  }
...
export async function updateVisibilityProjectStateForExecution(
  ...
  const { error } = await createSupabaseAdminClient()
    ...
    .eq("id", projectId);
  if (error) throw new Error(`Unable to update benchmark state: ${error.message}`);
}
...
export async function getVisibilityEvidence(
  ...
  const [observationsResult, citationsResult] = await Promise.all([
    supabase
      ...
      .select("run_id,answer_observed,brand_mentioned,competitor_mentions,recommendation_strength,ranked_position,confidence,claim_verified,signal")
      .in("run_id", runIds),
    supabase
      .from("visibility_citations")
...
export async function replaceVisibilityActions(
  ...
  if (clearError) throw new Error(`Unable to replace Action Queue: ${clearError.message}`);
  if (!actions.length) return;

  const { error: insertError } = await supabase.from("visibility_actions").insert(
    actions.map((action) => ({
      project_id: projectId,
  ...
  if (insertError) throw new Error(`Unable to save Action Queue: ${insertError.message}`);
}
...
const projectSelect =
  "id,brand_url,brand_name,description,primary_category,target_customers,key_use_cases,revenue_goal,markets,languages,surfaces,runtime_policy,state,created_at,updated_at";
const cardSelect =
...
const promptSelect =
  "id,topic_id,prompt_text,kind,buyer_realism,market,language,surfaces,why_selected,importance_score,competitor_entities,planned_samples,status";

export function formatVisibilityStorageError(error: unknown) {
  const message = error instanceof Error ? error.message : "Unable to store AI Visibility project.";
